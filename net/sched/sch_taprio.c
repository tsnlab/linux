...
static ktime_t get_cycle_time(struct sched_gate_list *sched)
{
	...
	ktime_t cycle = {.tv64 = 0};

	if (sched->cycle_time != 0)
		return (ktime_t){.tv64=sched->cycle_time};

	list_for_each_entry(entry, &sched->entries, list)
		cycle = ktime_add_ns(cycle, entry->interval);

	sched->cycle_time = cycle.tv64;
...
static enum hrtimer_restart advance_sched(struct hrtimer *timer)
{
	 ...
	 */
	if (unlikely(!entry || entry->close_time.tv64 == oper->base_time)) {
	...
	close_time.tv64 = min_t(s64, close_time.tv64, oper->cycle_close_time.tv64);
				 ...
				 struct sched_gate_list *new)
{
	...
	if (tb[TCA_TAPRIO_ATTR_SCHED_ENTRY_LIST])
		err = parse_sched_list(
			tb[TCA_TAPRIO_ATTR_SCHED_ENTRY_LIST], new);
	if (err < 0)
		return err;
				   ...
				   struct sched_gate_list *sched, ktime_t base)
{
	...
	sched->cycle_close_time = ktime_add_ns(base, cycle.tv64);
			       ...
			       ktime_t start, struct sched_gate_list *new)
{
	 ...
	 */
	start.tv64 = min_t(s64, start.tv64, expires.tv64);
...
static int taprio_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	err = parse_taprio_schedule(tb, new_admin);
	if (err < 0)
		goto free_sched;
